...
|------|-------|-----------|-------------|------|
| 2026-05-03 | 2.9.0 | 2.9.0 | feat/hixl-per-pair-region-aliasing | 合并 origin/main `7f129765d`（124 + 63 commits），架构切换：`monarch_types::UniqueId` 从 NCCL-only struct 改为 `enum {Nccl, Hccl}` 真 portable wire type；修上游 `64f3db3e8f` client.py 的 CUDA-only 短路以让 NPU 正确触发 `BackendNetworkInit`；跟进 hyperactor API rename（`hyperactor::reference::*` → `hyperactor::*`、`spawn=true` → `#[spawnable]`、`ActorId::new` 新签名） |
...
| 17 | 合并上游后 tensor_engine 所有集合通信报 `no comm found for stream ..., dims [npus]` | 上游 `64f3db3e8f`（"Enable tensor engine with CPU"）在 `python/monarch/common/client.py` 加了 `self._has_cuda = torch.cuda.is_available()` + `self._backend_network_init = not self._has_cuda` 短路，NPU 机器没 CUDA 所以 `BackendNetworkInit` 消息从不下发，Rust worker 的 `self.comm` 一直为 None | 保留 monarch1 的 `_npu_available()` helper，把判断改为 `self._has_accelerator = torch.cuda.is_available() or _npu_available()`；任何后续 rebase/merge 都要保留这条改动 |
| 18 | 合并上游后 `cargo check` 报 `method \`backend_network_init\` has an incompatible type for trait ... expected \`UniqueId\`, found \`RootInfo\`` 或 wire 反序列化报 length 128 | 上游把 `monarch_types::UniqueId` 写死成 128 字节 `NcclUniqueId` 包装；HCCL `RootInfo` 是 4108 字节，装不下 | 保留本仓的 enum 定义：`pub enum UniqueId { Nccl(NcclUniqueId), Hccl(HcclRootInfo) }`，以及 `torch-sys-ascend::hccl` 里的 `From<RootInfo> for UniqueId` / `TryFrom<UniqueId> for RootInfo` 双向转换；`monarch_extension/src/convert.rs` 的 `BackendNetworkInit` 构造要按 backend cfg 分叉（CUDA 走 `UniqueId::new_nccl()`，Ascend 走 `RootInfo::new().map(UniqueId::from)`）。上游该类型改动未来每次 merge 都会冲突，按 `249206ef6` 的 resolution 重放即可 |
| 19 | 合并上游后 `monarch_rdma` 编不过（`cannot find type \`RdmaManagerActor\``、`\`hyperactor::reference\` unresolved`、`\`spawn = true\` no longer supported`） | 上游 hyperactor API rename：`reference` 模块取消、`spawn = true` 属性取消、`ActorId::new` 新签名 | 跟随 upstream 的 ibverbs 变体的写法：`hyperactor::reference::X` → `hyperactor::X`；`#[hyperactor::export(spawn = true, ...)]` → `#[hyperactor::export(...)] + #[hyperactor::spawnable]`；`ActorRef::attest(ActorId::new(proc_id, name, 0))` → `ActorRef::attest(client.mailbox().actor_id().proc_ref().actor_id(name))` |
